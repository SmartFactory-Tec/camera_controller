...
class CameraSystem():
    def __init__(self, pid_x: tuple[float, float, float],
                 pid_y: tuple[float, float, float], enable_detection: bool, image_scale: float, controllable: bool, id: int | None = None,
        ...
        self.__enable_detection = enable_detection
    ...
    def get_latest_detections(self):
        if not self.__enable_detection: raise RuntimeError("Feature disabled!")
        if not self.__is_ready_flag.is_set(): return None
        with self.__detections_lock:
